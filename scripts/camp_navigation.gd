...
func setup(game:Node,return_to_camp:Callable)->void:
 ...
 var status:=Control.new();status.name="PotStatus";status.position=Vector2(76.5,98.4);status.size=Vector2(255,164);status.scale=Vector2(.4,.4);status.mouse_filter=Control.MOUSE_FILTER_IGNORE;add_child(status)
 var cooking:bool=not game.pending_stew.is_empty()
 var ready:bool=not game.completed_stew_result.is_empty()
 var view_container:=SubViewportContainer.new();view_container.name="PotStatusIcon";view_container.position=Vector2(47,24);view_container.size=Vector2(160,140);view_container.mouse_filter=Control.MOUSE_FILTER_IGNORE;status.add_child(view_container)
 ...
 if cooking:
  ...
  var count:Label=game.label(status,"%d/%d"%[required-remaining,required],Vector2(0,0),22,Color.WHITE,true,HORIZONTAL_ALIGNMENT_CENTER,255);count.name="PotExpeditionCount";count.size.y=30;count.add_theme_color_override("font_outline_color",Color("#20382d"));count.add_theme_constant_override("outline_size",6)
 elif ready:
  var badge:Panel=game.panel(Rect2(111,0,34,34),Color("#f15f9b"),17);badge.name="PotReadyBadge";badge.mouse_filter=Control.MOUSE_FILTER_IGNORE;status.add_child(badge)
