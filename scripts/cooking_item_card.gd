...
func setup(icon_text:String,title:String,subtitle:String,count:int,color:Color,payload:Dictionary)->void:
	...
	if str(payload.get("kind",""))=="spice":
		...
		if not texture_path.is_empty() and ResourceLoader.exists(texture_path):
			...
			var fallback:=Label.new();fallback.name="SpiceSymbol";fallback.text=icon_text;fallback.position=Vector2(5,7) if detailed else Vector2(12,4);fallback.size=Vector2(38,38) if detailed else Vector2(43,43);fallback.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER;fallback.vertical_alignment=VERTICAL_ALIGNMENT_CENTER;fallback.add_theme_font_size_override("font_size",23 if detailed else 27);fallback.add_theme_color_override("font_color",color.darkened(.2));fallback.mouse_filter=Control.MOUSE_FILTER_IGNORE;add_child(fallback)
		if detailed:
