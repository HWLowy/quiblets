...
func run()->void:
	...
	game.move_stone_inventory.clear();game.move_stone_inventory["echo"]=18
	...
	game.selected_roster=0;game.stone_inventory_filter="health";game.show_quiblet_edit();await process_frame
	check(game.content.find_children("StoneInventoryTab_*","Button",true,false).size()==3,"The equipment inventory needs exactly three category tabs")
	var tab:Button=game.content.find_child("StoneInventoryTab_health",true,false);var grid:GridContainer=game.content.find_child("StoneIconGrid",true,false);var navigation:Control=game.content.find_child("PageNavigation",true,false);var panel:Panel=game.content.find_child("StoneInventory",true,false)
	...
	check(game.stone_inventory_filter=="health" and not game.selected_inventory_item.is_empty(),"Selecting a Health stone should keep the Health tab and its detail")
	game.set_stone_inventory_filter("attack");await process_frame;visible=cards(game)
	...
	check(game.stone_inventory_filter=="move" and game.content.find_child("StoneDetailTitle",true,false).text=="Echo Stone","Inspecting a fitted Move Stone should automatically reveal the Move tab")
